...
for month, ds_month in ds.groupby('time.month'):
    for year, ds_month_year in ds_month.groupby('time.year'):
        ...
        if os.path.exists(out_file):
            ...
            new_times = ds_month_year['time'].values
            has_new_times = not all(np.isin(new_times, existing_times))
            ...
            if has_new_times or has_new_vars:
                # If we have new times, merge along time dimension
                if has_new_times:
                    # For variables in both datasets
                    common_vars = existing_vars.intersection(new_vars)
                    combined_ds = xr.Dataset()
                    
                    # Process common variables - concatenate along time
                    for var in common_vars:
                        combined_data = xr.concat([existing_ds[var], ds_month_year[var]], dim='time')
                        combined_data = combined_data.sortby('time').drop_duplicates(dim='time')
                        combined_ds[var] = combined_data
                    
                    # Add variables only in existing dataset
                    for var in existing_vars - common_vars:
                        combined_ds[var] = existing_ds[var]
                        
                    # Add variables only in new dataset
                    for var in new_vars - existing_vars:
                        # For new variables, we need to ensure they have values for all times
                        # This might require filling with NaN values for times not in ds_month_year
                        if has_new_vars and 'time' in ds_month_year[var].dims:
                            print(f"Adding new variable: {var}")
                            combined_ds[var] = ds_month_year[var]
                            print(f"New time in {year}{month}")
                else:
